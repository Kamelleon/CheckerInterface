...
class GUIManager:
    ...
    def initialize_window(self, layout):
        try:
            ...
            logger.info("Utworzono okno interfejsu")
        except:
            logger.critical(f"Wystapil problem podczas tworzenia okna interfejsu brakarza. Opis bledu {traceback.format_exc()}")
            os._exit(1)
        self.handle_window()
    ...
    def handle_window(self):
        ...
        while True:
            try:
                ...
                logger.critical(f"Wystapil nieznany dotad problem w interfejsie brakarza. Opis bledu {traceback.format_exc()}")
                os._exit(1)
    ...
    def get_images(self):
        ...
        while not folder_found:
            try:
                ...
                logger.info(f"Znaleziono najnowszy folder z data w nazwie ktory powinien zawierac zdjecia ({latest_directory})")
                if not os.path.exists(gui_window.complete_path):
                    try:
                        os.system(f"sudo mkdir {gui_window.complete_path}")
                        logger.info(f"Utworzono folder z kolorem: {gui_window.complete_path}")
                        print("[+] Utworzono folder z kolorem")
                    except:
                        print(traceback.print_exc())
                        logger.error(
                            f"Wystapil problem podczas proby utworzenia folderu z kolorem: {gui_window.complete_path}. Opis bledu {traceback.format_exc()}")
                        os._exit(1)
                folder_found = True
            except:
                print(traceback.print_exc())
                logger.error(f"Nie mozna bylo znalezc podanego folderu: '{path_to_directories_with_brick_image_dates}' Opis bledu {traceback.format_exc()}")
        ...
        try:
            ...
            print(traceback.print_exc())
            logger.error(f"Wystapil inny problem podczas pobierania plikow ostatnich zdjec z folderu. Opis bledu {traceback.format_exc()}")
            os._exit(1)
...
if __name__ == "__main__":
    ...
    if platform.system() == "Linux":
        logging.basicConfig(filename='/home/pi/aparat/logs/brakarz_logger.log', filemode='a', format='[%(asctime)s - %(name)s - %(levelname)s] %(message)s')
    else:
        logging.basicConfig(filename='brakarz_logger.log', filemode='a', format='[%(asctime)s - %(name)s - %(levelname)s] %(message)s')
    logger = logging.getLogger('Interfejs_Brakarza')
    logger.setLevel(logging.DEBUG)
    logger.info("Uruchomiono skrypt")
    logger.info("Inicjowanie zmiennych...")
    user = getpass.getuser()
    manager = ImageManager()
    gui_window = GUIManager("Interfejs_brakarza")
    BUTTON_SIZE = (12, 5)
    BUTTON_SIZE_FOR_COLORS = (9,3)
    FONT_FOR_COLORS = ("Helvetica", 19)
    latest_image = None
    one_before_latest_image = None
    folder_found = False
    logger.info("Zainicjowano zmienne")
    ...
    if platform.system() == "Linux" and os.environ.get('DISPLAY', '') == '':
        try:
            ...
            logger.error(f"Problem ze znalezieniem wyswietlacza. Opis bledu: {traceback.format_exc()}")
            time.sleep(0.5)
            print(traceback.print_exc())
            # os._exit(1)
    ...
    def main():
        time.sleep(1)
        logger.info("Zainicjowano watek 'main'")
        while True:
            try:
                if not gui_window.stop_checking:

                    try:
                        logger.info("Pobieranie zdjec z odpowiednich folderow.")
                        image1, image2 = manager.get_images()
                        logger.info(f"Pobrano zdjecia z odpowiednich folderow (image1: '{image1}', image2: '{image2}')")
                    except:
                        print(f"Wystapil problem podczas pobierania zdjeć. Opis bledu {traceback.format_exc()}")
                        logger.error(f"Wystapil problem podczas pobierania zdjeć. Opis bledu {traceback.format_exc()}")
                        time.sleep(2)
                        continue
                    ...
                    try:
                        ...
                        logger.error(f"Wystapil problem podczas wczytywania zdjec przez OpenCV. Opis bledu {traceback.format_exc()}")
                        time.sleep(2)
                        continue

                    try:
                        print(f"[~] Porownywanie zdjec:\nImage1: '{image1}'\nImage2: '{image2}')")
                        logger.info("Porownywanie zdjec...")
                        compare_result = manager.compare_images(img1_crop, img2_crop)
                        logger.info("Porownano zdjecia")
                    except:
                        print(f"Wystapil problem podczas porownywania zdjec. Opis bledu {traceback.format_exc()}")
                        logger.error(f"Wystapil problem podczas porownywania zdjec. Opis bledu {traceback.format_exc()}")
                        time.sleep(2)
                        continue

                    if compare_result == True:
                        gui_window.brick_clicked = False
                        logger.info("To ten sam produkt")
                        print("[!] To ten sam produkt")
                        print(f"[~] Kopiuje zdjecie do folderu: {gui_window.complete_path}")
                        logger.info("Kopiowanie zdjecia do odpowiedniego folderu...")
                        try:
                            if platform.system() == "Linux":
                            ...
                            print("[+] Skopiowano")
                            logger.info("Skopiowano zdjecie do odpowiedniego folderu")
                        except:
                            print(f"Wystapil problem podczas kopiowania zdjecia do odpowiedniego folderu. Opis bledu {traceback.format_exc()}")
                            logger.error(f"Wystapil problem podczas kopiowania zdjecia do odpowiedniego folderu. Opis bledu {traceback.format_exc()}")
                            continue
                        ...
                        logger.warning("Nie kopiuje zdjecia")
                        ...
                        while True:
                            ...
                            if gui_window.brick_clicked:
                                logger.info("Kliknieto w graficzny interfejs")
                ...
                print(f"[-] Wystapil inny nieznany dotad problem w watku 'main'. Opis bledu {traceback.format_exc()}")
                logger.critical(f"Wystapil inny nieznany dotad problem w watku 'main'. Opis bledu {traceback.format_exc()}")
                os._exit(1)
    ...
    gui_window.initialize_window([[
            sg.Column([
                ...
                [sg.Button('Kostka\nbrukowa', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Pustaki', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Daszki', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button("Gazony", size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button("Obrzeza", size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button("Stopnie", size=BUTTON_SIZE, font=("Helvetica", 17))],
                 [sg.Button("Palisady", size=BUTTON_SIZE, font=("Helvetica", 17))]],
                key='-RODZAJ_PRODUKTU-', visible=True),
            sg.Column([
                [sg.Text('2. Wybierz kształt kostki:', font=("Helvetica", 25))],
                [sg.Button('Bruk\nprosty', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Cegielka', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Fantazja', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Grand', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Kostka\nintegracyjna', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Master', size=BUTTON_SIZE, font=("Helvetica", 17))],
                [sg.Button('Master\nXL', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Metro', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Metro\nXL', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Mosaic', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Perfect', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Podwojne\nT', size=BUTTON_SIZE, font=("Helvetica", 17))],
                [sg.Button('Smart', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Starobruk', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Trend', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Vjetra', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Wena', size=BUTTON_SIZE, font=("Helvetica", 17))]],
                key='-KSZTALT_KOSTKI-',visible=False),
            sg.Column([
                [sg.Text('2. Wybierz kształt płyty:', font=("Helvetica", 25))],
                [sg.Button('Cube', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Ecolinea', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Ecosolid', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Linea', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Longer', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Magnum', size=BUTTON_SIZE, font=("Helvetica", 17))],
                [sg.Button('Multi', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Plyta\nazurowa', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Simple', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Solid', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Style', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Unique\nstyle', size=BUTTON_SIZE, font=("Helvetica", 17))],
                [sg.Button('Viva', size=BUTTON_SIZE, font=("Helvetica", 17))]],
                key='-KSZTALT_PLYTY-', visible=False),
            sg.Column([
                [sg.Text('2. Wybierz kształt pustaka:', font=("Helvetica", 25))],
                [sg.Button('Claro', size=BUTTON_SIZE, key="Claro1", font=("Helvetica", 17)),
                 sg.Button('Murowy\nstructur', size=BUTTON_SIZE, font=("Helvetica", 17)),
                 sg.Button('Slupkowy\nstructur', size=BUTTON_SIZE, font=("Helvetica", 17))]],
                key='-KSZTALT_PUSTAKOW-', visible=False),
        sg.Column([
            [sg.Text('2. Wybierz kształt daszka:', font=("Helvetica", 25))],
            [sg.Button('Claro', size=BUTTON_SIZE, key="Claro0", font=("Helvetica", 17)),
             sg.Button('Dwuspadowy\nstructur', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Plaski\nstructur', size=BUTTON_SIZE, font=("Helvetica", 17))]],
            key='-KSZTALT_DASZKOW-', visible=False),
        sg.Column([
            [sg.Text('2. Wybierz kształt gazona:', font=("Helvetica", 25))],
            [sg.Button('Agaflor', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Lucaflor', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Patiflor', size=BUTTON_SIZE, font=("Helvetica", 17))]],
            key='-KSZTALT_GAZONOW-', visible=False),
        sg.Column([
            [sg.Text('2. Wybierz kształt obrzeża:', font=("Helvetica", 25))],
            [sg.Button('Palisadowe', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Palisadowe\nXL', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('8x30x100', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button("8x25x100", size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button("6x20x100", size=BUTTON_SIZE, font=("Helvetica", 17))]],
            key='-KSZTALT_OBRZEZY-', visible=False),

        sg.Column([
            [sg.Text('2. Wybierz kształt palisady:', font=("Helvetica", 25))],
            [sg.Button('Palisada\nkwadratowa', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Palisada\nokragla', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Palisada\nsplitowana', size=BUTTON_SIZE, font=("Helvetica", 17))]],
            key='-KSZTALT_PALISAD-', visible=False),

        sg.Column([
            [sg.Text('2. Wybierz kształt stopnia:', font=("Helvetica", 25))],
            [sg.Button('Stopien', size=BUTTON_SIZE, font=("Helvetica", 17)),
             sg.Button('Stopien\nsplitowany', size=BUTTON_SIZE, font=("Helvetica", 17))]],
            key='-KSZTALT_STOPNI-', visible=False),

            sg.Column([
                [sg.Text('3. Wybierz kolor wskazanego produktu:', font=("Helvetica", 25))],
                [sg.Button('Ambra', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Antracyt', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Antracyt\nmelanz', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Antracytowy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Barwy\njesieni', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Bianco', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Bialy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)],
                 [sg.Button('Bronzo', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Brazowy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Carbon', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Carbon\nnatural', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Concrete\nnatural', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Corten', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Czerwony', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)],
                 [sg.Button('Dark', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Diamond', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Giallo', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Gothic', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Grafit', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Grafitowy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Granito', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)],
                 [sg.Button('Gray', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Light', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Mokka', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Muszlowy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Nero', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Piaskowy', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Piaskowy\nmelanz', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)],
                 [sg.Button('Popielaty', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Steel', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Steel\nnatural', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Style\ncarbon', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Style\ngray', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Style\nsteel', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Style\nwhite', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)],
                [sg.Button('Szary', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('Vero', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('White', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('White\nnatural', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS),
                 sg.Button('York', size=BUTTON_SIZE_FOR_COLORS, font=FONT_FOR_COLORS)]
            ],
            ...
            sg.Column([
                                        ...
                                        [sg.T("", pad=(210, 1)), sg.Text("Rodzaj:", font=("Helvetica", 33)), sg.Text("brak_danych", key="-TEXT-RODZAJ-PRODUKTU-", font=("Helvetica", 33))],
                                        [sg.T("", pad=(210, 10)),sg.Text("Kształt:", font=("Helvetica", 33)),sg.Text("brak_danych",key="-TEXT-KSZTALT-PRODUKTU-", font=("Helvetica", 33))],
                                        [sg.T("", pad=(210, 10)),sg.Text("Kolor:  ", font=("Helvetica", 33)),sg.Text("brak_danych",key="-TEXT-KOLOR-PRODUKTU-", font=("Helvetica", 33))],
                                        [sg.B("Zmień produkt", size=(15, 4), font=("Helvetica", 28), pad=(430, 40))],
                                        [sg.T("", font=("Helvetica", 100))]
                                    ],
                visible=False, key='-OK-')]])

    logger.info("Zainicjowano graficzny interfejs brakarza")
